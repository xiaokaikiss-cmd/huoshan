...
export class AssetService {
  private getAuthToken(): string {
    const token = process.env.WETOKEN_API_KEY;
    if (!token) {
      throw new Error('WETOKEN_API_KEY 环境变量未配置');
    }
    return token;
  }
  ...
  async syncAssetStatus(id: string): Promise<Asset> {
    ...
    const remoteAsset = await this.queryAssetStatus(localAsset.asset_id);
   ...
   * 批量同步处理中的素材
   */
  async syncProcessingAssets(): Promise<void> {
    const client = getSupabaseClient();
    ...
    for (const asset of processingAssets) {
      try {
        const remoteAsset = await this.queryAssetStatus(asset.asset_id);
